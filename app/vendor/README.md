...
Downloaded 29 Aug 2026 12:54 IST. Moved under `app/` on 29 Aug 13:45 IST: `llama-server
--path ~/safetyeye/app` serves only what is inside `app/`, so a sibling `vendor/` at the
repo root was unreachable at runtime — `../vendor/...` resolves above the document root
and 404s.
